...
- 5dc5e18: Dubbing: the free trial is now a 15-second preview, and the result says so.
...
- 81fab6f: Proofread: transcribe a video and translate the transcript into editable
  subtitle files.
...
- 434d7f1: Document Hindi as a dubbing target.
...
- 4289f83: Document Tamil, Malayalam, Kannada, Gujarati, Punjabi and Sindhi as dubbing targets.
...
- ad1e321: `duration` is optional on text-to-music and text-to-sfx.
...
- c36967d: Dubbing takes subtitle scripts and returns re-timed SRTs.
  ...
  language, and `--export-srt`, which writes each re-timed file beside its video
  (`clip.es.mp4` → `clip.es.srt`) and prints the preflight and export status of
  every language. An `--output` ending in `.srt` is refused with `--export-srt`,
  since the subtitle would otherwise be written over the dubbed video.

### Patch Changes

- Updated dependencies [c36967d]
  - sonilo@0.18.0
...
- c7d2f58: Document `ar`, `tr`, `vi` and `id` as dubbing targets.
...
- a9cec3e: Document `pt_br`, `es_419` and `th` as dubbing targets.
...
- a4ecaeb: Add `sonilo login`, `sonilo logout` and `sonilo whoami`. Signing in stores a
