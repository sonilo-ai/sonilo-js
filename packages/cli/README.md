...
sonilo video-analysis --video clip.mp4 --mode sfx
```
...
brief**. `--mode` picks which brief comes back: `both` (the default) returns the music brief
(`segments` / `variations`) and a sound-effects brief (`sfx_segments` / a single `sfx_prompt`)
in one call; `music` or `sfx` returns only that one, at the same price. Source videos may be
at most 480 seconds long, and billing has a 10-second floor.
